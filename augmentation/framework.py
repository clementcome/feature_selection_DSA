...
class Framework:
    ...
    def get_overlappings(
        ...
        query = (
            "SELECT SUBQ.ids FROM (SELECT table_col_id AS ids,"
    ...
    @timer
    def table_max_column(
        self, table_id_list: List[int], connection: DBConnection = None
    ) -> Dict[int, int]:
        """
        ...
        cur.execute(
            "SELECT tableid, maxcol from cbi_inverted_index_2 WHERE tableid IN ('{}');".format(
                "','".join(s)
            )
        )
    ...
    def get_clean_dataset(
        self, data_path: str, query_column: str, target_column: str
    ) -> pd.DataFrame:
        """
        ...
        data[query_column] = data[query_column].apply(self.get_cleaned_text)
        # Only supports regression until now
        data[target_column] = data[target_column].astype(float)
        return data

    @timer
    def get_external_table_dict(
        self, table_id_list: List[int], connection: DBConnection = None
        ...
        connection : DBConnection, optional
            Connection to the database. If not provided, the connection will be initialized,
            by default None
        ...
        Dict[int, Dict[int, List[str]]]
            ...
            result[table_id][row_id] is the list of the values contained in 
            the row in the table designated by column_id and table_id
        """
        ...
        external_tables = pd.DataFrame(
            cur.fetchall(),
            columns=["tableid", "colid", "rowid", "table_row_id", "tokenized"],
        )

        temp = (
            external_tables.sort_values(by=["tableid", "rowid", "colid"])
            .groupby(["tableid", "rowid"])
            .tokenized.apply(list)
            .reset_index()
        )
    ...
    @timer
    def get_external_table_cleaned(
        self,
        table_id: int,
    ...
    def perform_join(
        self,
        table_and_col_to_keep: Dict[int, List[int]],
        col_id_dict: Dict[int, int],
        external_dict: Dict[int, Dict[int, List[str]]],
        data: pd.DataFrame,
        query_column: str,
        ...
        """
        Saves and returns the base dataset joined with the columns from the tables in table_and_col_to_keep

        Parameters
        ----------
        table_and_col_to_keep : Dict[int, List[int]]
            keys are table_ids and values are the list of the column_ids to join from this table
        col_id_dict : Dict[int, int]
            Value is the column on which to perform the join for the table designated by the key
        external_dict : Dict[int, Dict[int, List[str]]]
            External tables dictionary, output from get_external_table_dict
        data : pd.DataFrame
            The base dataset, output from get_dataset_cleaned
        query_column : str
            Column of the base dataset on which to perform the join
        data_path : str
            Path to the base dataset. 
            This is used to generate the name of the file in which the joined dataset is saved
        k : int
            Number of table to consider for the join. 
            This is used to generate the name of the file in which the joined dataset is saved

        Returns
        -------
        pd.DataFrame
            Joined dataset of the base dataset and the external tables 
        """
    ...
    def run(
        self, data_path: str, query_column: str, target_column: str, k: int
    ) -> pd.DataFrame:
        """
        Performs the whole augmentation on the base dataset
        ...
        target_column : str
            Column of the base dataset used as target
        k : int
            Number of tables to consider for joining
